...
def ft_inventory_system():
    ...
    for item_name, qty in items.items():
        ...
        print(f"{item_name} ({type}, {rarity}): {qty}x @ {value} gold each = {total_value} gold")
    ...
    if giver_items.get(obj, 0) >= qty_obj:
        ...
        print(f"\n=== Transaction: {giver.capitalize()} gives {receiver.capitalize()} {qty_obj} {obj} ===")
    ...
    print("\n=== Inventory Analytics ===")
    print(f"Most valuable player: {most_valuable_name.capitalize()} ({most_valuable_value} gold)")
    print(f"Most items: {most_items_name.capitalize()} ({most_items_count} items)")
    ...
    rarest = [item for item, count in item_counts.items() if count == 1]
    print("Rarest items:", end=" ")
    for i, item in enumerate(rarest):
        if i > 0:
            print(", ", end="")
        print(item, end="")
    print()
